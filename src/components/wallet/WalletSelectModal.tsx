...
export function WalletSelectModal({ open, onOpenChange }: WalletSelectModalProps) {
  ...
  const handleConnect = async (wallet: typeof WALLET_OPTIONS[0]) => {
    ...
    // For browser extension wallets, check installation
    ...
    if (wallet.type === 'crossmark') {
      ...
      // Xaman is a mobile wallet, not a browser extension
      // Skip installation check - will show QR code for connection
      isInstalled = true;
    }
